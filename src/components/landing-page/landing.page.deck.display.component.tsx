    ...
    render() {
        ...
        return (
            <Card key={`deck-` + deck.id} style={{ backgroundColor: '#333', borderColor: '#333' }}  >
                ...
                <small className="text-muted">Artist: {deck.author}</small>
                <Collapse background-color="dark" isOpen={this.state.collapse}>
                    ...
                    <Card body inverse color="dark" >
                        <CardTitle>Holder Name: {deck.name}</CardTitle>
                        <CardSubtitle>Holder Author: {deck.author}</CardSubtitle>
                        <CardSubtitle>Holder Format: {deck.format}</CardSubtitle>
                        {deck.description && <CardText>Holder Description: {deck.description}</CardText>}
                    </Card>
                </Collapse>
            </Card>
