...
interface IDeckUpdateTogglableProps {
    deck: Deck
...
interface IDeckUpdateTogglablestate {
    showMainboard: boolean
    randomCardNumber: number[]
    randomCard: string[]
}

export default class DeckUpdateTogglable extends Component<IDeckUpdateTogglableProps, IDeckUpdateTogglablestate> {
    constructor(props: IDeckUpdateTogglableProps) {
        super(props);
    ...
    setDefaultInputFormat = (main: boolean) => {
        let placeholder = '';
        let arr: any[]
        if(main){
            arr = this.props.deck.mainboard
        } else {
            arr = this.props.deck.sideboard
        }
        arr.forEach(ele => {
            placeholder += `${ele}\n`
        })
